...
def plot():
    ...
    for plot_num in range(5):
        for graphon in ['power']:
            ...
            plt.subplot(1, 5, i)
            plt.gca().text(-0.01, 1.06, '(' + string.ascii_lowercase[5+i-1] + ')', transform=plt.gca().transAxes,
                size=22, weight='bold')
            i += 1
            ...
            if plot_num <= 3:
                ...
                cb1.set_label(r'$\pi^\alpha_t(1 \mid \textrm{Pri}'+fr'{state_label[plot_num]})$')
            else:
                plt.xlabel(fr'Time $t$')
                plt.ylabel(r'Infection prob. $\mu^\alpha_t(I \mid \textrm{Pri})$')
                # plt.ylim([0, 1])
                plt.xlim([0, 49])
    ...
    plt.tight_layout()
    plt.savefig('./figures/Cyber_Het_Pri_heatmap.pdf', bbox_inches='tight', transparent=True, pad_inches=0)
    plt.savefig('./figures/Cyber_Het_Pri_heatmap.png', bbox_inches='tight', transparent=True, pad_inches=0)
    plt.show()
