...
def plot():
    ...
    for plot_num in range(5):
        for graphon in ['power']:
            ...
            with open(args['experiment_directory'] + 'logs.pkl', 'rb') as f:
                ...
                if plot_num < 4:
                    ...
                    for alpha_idx in range(len(plotted_alphas)):
                        means = []
                        for t in range(50):
                            means.append(policy.pmf(t, tuple([plotted_alphas[alpha_idx], 4+plot_num]))[1])
                    ...
                    for alpha_idx in range(len(plotted_alphas)):
                        means = []
                        for t in range(50):
                            mean = np.array(result[-1]['simulator']['mus'][t][alpha_idx][4+2]) \
                                   + result[-1]['simulator']['mus'][t][alpha_idx][4+0]
                            means.append(2 * mean)
            ...
            if plot_num <= 3:
                ...
                cb1.set_label(r'$\pi^\alpha_t(1 \mid \textrm{Cor}'+fr'{state_label[plot_num]})$')
            else:
                plt.xlabel(fr'Time $t$')
                plt.ylabel(r'Infection prob. $\mu^\alpha_t(I \mid \textrm{Cor})$')
                # plt.ylim([0, 1])
                plt.xlim([0, 49])
    ...
    plt.tight_layout()
    plt.savefig('./figures/Cyber_Het_Cor_heatmap.pdf', bbox_inches='tight', transparent=True, pad_inches=0)
    plt.savefig('./figures/Cyber_Het_Cor_heatmap.png', bbox_inches='tight', transparent=True, pad_inches=0)
    plt.show()
