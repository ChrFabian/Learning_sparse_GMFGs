...
class UniformGraphonNPlayerSimulator:
    ...
    def reset(self, fixed_alphas=None):
        ...
        for i in range(self.n_players):
            for j in range(self.n_players):
                if j > i:
                    edge_prob = min(1., self.mfg.graphon(self.x[i][0], self.x[j][0]) * self.rho_n)
        ...
        for node_index in range(self.n_players):
            observation = self.augment_obs(self.t + 1, self.mfg.observation(self.t, None,
                                                                            None, None, self.x[node_index]))
    ...
    def get_masked_mu(self, x, node_index):
        ...
        return NormalizedParticleMeanField(self.mfg.agent_observation_space, masked_x, self.n_players * self.rho_n)
